...
class Task():
    ...
    def __repr__(self):
        if self.agent == g_human_name:
            return "{}-{}-{}{}".format(self.id, "H", self.name, self.parameters)
        elif self.agent == g_robot_name:
...
class Operator(Task):
    ...
    def __init__(self, name, parameters, agent, why, decompo_number, function=None):
    ...
    def insert_com_action(divergences, current_cost, aligned_agents, previous_action):
        ...
        com_action = Operator("COM_ALIGN", com_info, g_robot_name, None, 0)
        ...
        aligned_agents[g_robot_name].planned_actions.append(
    ...
    def apply(agents, action_task, acting_agent, agent_state_in, agent_state_out):
        ...
        if g_with_contrib and opAg.donecond!=None and opAg.donecond(agents, state_in, acting_agent, *params):
        ...
        if opAg.precond!=None and not opAg.precond(agents, state_in, acting_agent, *params):
            print(bcolors.WARNING + str(action_task) + " not applicable" + bcolors.ENDC)
...
class BeliefDivergence:
    ...
    def in_agents(self, agents):
        f_r = agents[g_robot_name].state.get_fluent(self.property)
        f_h = agents[g_human_name].state.get_fluent(self.property)
...
class BeliefDivergences:
    ...
    def update(self, newagents):
        ...
        self.divergences = []
        for fluent_name in newagents[g_robot_name].state.fluent_names:
            r_fluent = newagents[g_robot_name].state.get_fluent(fluent_name)
            h_fluent = newagents[g_human_name].state.get_fluent(fluent_name)
...
class Agents:
    ...
    def get_fluent_loc(self, fluent_name):
        return self.agents[g_robot_name].state.get_fluent(fluent_name).loc
    
    def set_fluent_loc(self, fluent_name, loc):
        self.agents[g_robot_name].state.get_fluent(fluent_name).loc = loc

    def get_fluent_obs(self, fluent_name):
        return self.agents[g_robot_name].state.get_fluent(fluent_name).obs

    def set_fluent_obs(self, fluent_name, obs):
        if type(obs) != ObsType:
            raise Exception("set_fluent_obs: {} not of {} type!".format(obs, ObsType))
        self.agents[g_robot_name].state.get_fluent(fluent_name).obs = obs
...
def print_agent(agent, with_static=False):
    print("\n|AGENDA {} = ".format(agent))
    for t in g_agents[agent].agenda:
        print("||\t-{} {}".format(t.name, t.parameters))
...
def print_agendas(agents):
    print("| AGENDA {} =".format(g_robot_name))
    for t in agents[g_robot_name].agenda:
        print("||\t-{}".format(t))
    print("| AGENDA {} =".format(g_human_name))
    for t in agents[g_human_name].agenda:
        print("||\t-{}".format(t))
...
def print_agents_step(agents, with_static=False):
    ...
    print("|------------------------------------------------------------------------")
    print("| STATE {} =".format(g_robot_name))
    print_state(agents[g_robot_name].state, with_static)
    print("| STATE {} =".format(g_human_name))
    print_state(agents[g_human_name].state, with_static)
...
def print_operators(agent=None):
    ...
    if agent is None:
        print("==OPERATORS==")
        for a, ag in g_agents.agents.items():
            print("Agent:", a)
            print("\t", ', '.join(ag.operators))
    else:
        print('OPERATORS:', ', '.join(g_agents[agent].operators))
...
def heuristic_exploration(n_plot):
    ...
    if g_compute_gui:
        n_plot += 1
        hatpehda.gui.show_tree(first_node, str(n_plot), view=g_view_gui)
        hatpehda.gui.show_all(get_last_nodes_action(first_node), g_robot_name, g_human_name, 0, g_with_contrib, with_begin="false", with_abstract="false")
    if g_stop_input:
        input()
    ...
    while len(e_flagged_nodes)>0:
        ...
        # Pick the cheappest E node to explore
        picked_node = pick_cheapest_node(e_flagged_nodes)
        if g_debug:
            print("\n<@> picked node = ", end="")
            show_node(picked_node)
        ...
        if g_debug:
            ...
            show_nodes(new_e_flagged_nodes)
            print("e_flagged_nodes :")
            show_nodes(e_flagged_nodes)
        ...
        if g_debug:
            ...
            show_node(Ns)
        # Seek the cheapest final node (Nf) from the new E flagged nodes
        Nf = pick_cheapest_final_node(new_e_flagged_nodes)
        if g_debug:
            print("Nf = ", end="")
            show_node(Nf)
        # If there is a new final node
        ...
        if g_compute_gui:
            n_plot+=1
            hatpehda.gui.show_tree(first_node, str(n_plot), view=g_view_gui)
            hatpehda.gui.show_all(get_last_nodes_action(first_node), g_robot_name, g_human_name, 0, g_with_contrib, with_begin="false", with_abstract="true")
        if g_stop_input:
            input()
...
def explore_node(node, e_flagged_nodes, new_e_flagged_nodes, u_flagged_nodes, Ns):
    ...
    while True:
        ...
        if previous_action.agent == g_robot_name:
            next_agent_name = g_human_name
            next_agent_name_short = "H"
        elif previous_action.agent == g_human_name:
            next_agent_name = g_robot_name
        ...
        if node.type==Node.Type.D and newagents.copresent():
        ...
        # Branching for delay
        if rel_divs!=None and g_with_delay:
        ...
        if len(applied_refinement.decompos)>1:
            # Update current node (depth, type H/R, flag F)
            if previous_action.agent == g_robot_name:
                node.type = Node.Type.H
            elif previous_action.agent == g_human_name:
                node.type = Node.Type.R
            node.flag = Node.Flag.F
            ...
            for decomp in applied_refinement.decompos:
                ...
                if decomp.type == DecompType.BOTH_AGENDAS_EMPTY:
                    # Update current to solution (type F, flag E, update e flag lists)
                    node.type = Node.Type.F
                    node.flag = Node.Flag.E
                    new_e_flagged_nodes.append(node)
                else:
                    # Create incomplete node (previous, cost, depth, next, flag E)
                    next_node = create_incomplete_node_dec(decomp, node, current_cost+decomp.subtasks[0].cost, node.depth)
                    node.next_nodes.append(next_node)

                    if checkInactivityDeadlockDec(decomp):
                        node.Flag = Node.Flag.D
                    else:
        ...
        # Elif there is only one decomposition
            ...
            # END4- If inactivity deadlock is detected
                ...
                node.Flag = Node.Flag.D
                print(bcolors.WARNING + "## ERROR ## Inactivity deadlock detected." + bcolors.ENDC)
...
def create_delay_branch(node, rel_divs, newagents, previous_action, current_cost, e_flagged_nodes, new_e_flagged_nodes, u_flagged_nodes, phase):
    ...
    if div.in_agents(initial_agents):
        ...
        if action.name == "BEGIN":
            ...
            # print("div_node.end_agents[g_other_agent_name[action.agent]].planned_actions=", div_node.end_agents[g_other_agent_name[action.agent]].planned_actions)
            for a in action.next:
                if a.id == div_node.end_agents[g_other_agent_name[action.agent]].planned_actions[0][0]:
                    action = a
                    break
        ...
        while not div_action_found:
            ...
            print("\t\tR-Agenda in {}".format(action.agent_agendas[g_robot_name]))
            print("\t\tH-Agenda in {}".format(action.agent_agendas[g_human_name]))
            ...
            if action.name == "COM_ALIGN":
                ...
                print(prop, key, val)
                f = find_agents[g_human_name].state.get_fluent(prop)
                try:
                    f.val = int(val)
                except ValueError:
                    f.val = val
            ...
            print("after acting")
            print_agents_step(find_agents)
            if g_robot_name==acting_agent:
                apply_inferable_effects(find_agents, action)
            elif g_human_name==acting_agent:
                Operator.apply(find_agents, action, acting_agent, agent_state_in=g_other_agent_name[acting_agent], agent_state_out=g_other_agent_name[acting_agent])
            print("after other")
            print_agents_step(find_agents)
            # check if div
        ...
        div_action = action
        print("agendas:")
        print("div_action.agent_agendas[g_human_name]=", div_action.agent_agendas[g_human_name])
        print("div_action.agent_agendas[g_robot_name]=", div_action.agent_agendas[g_robot_name])
        agents_before_div[g_human_name].agenda = div_action.agent_agendas[g_human_name]
        agents_before_div[g_robot_name].agenda = [div_action] + div_action.agent_agendas[g_robot_name]
        ...
        # Delay node
        delay_current_cost = div_action.previous.current_plan_cost + g_delay_cost[g_robot_name]
        delay_action = Operator.create_inactivity_op("DELAY", g_delay_cost[g_robot_name], g_robot_name, div_action.previous, delay_current_cost, branching_name="delaying")
        end_agents_delay = deepcopy(agents_before_div)
        end_agents_delay[g_robot_name].planned_actions.append( (delay_action.id, delay_action.name, delay_action.agent, delay_action.parameters) )
        delay_node = Node()
        ...
        delay_node.type = Node.Type.D
        delay_node.flag = Node.Flag.E
        ...
        div_node.type = Node.Type.R
        div_node.flag = Node.Flag.F
...
def kill_delay_branch(node):
    n = node
    while n.previous_node.type==Node.Type.D:
        n = n.previous_node
...
def refine_u_nodes(first_node, u_flagged_nodes, n_plot):
    while u_flagged_nodes!=[]:
        ...
        node = pick_cheapest_node(u_flagged_nodes)
        ...
        if g_compute_gui:
            n_plot+=1
            hatpehda.gui.show_tree(first_node, str(n_plot), view=g_view_gui)
            hatpehda.gui.show_all(get_last_nodes_action(first_node), g_robot_name, g_human_name, with_begin="false", with_abstract="true")
        if g_stop_input:
            input()
        ...
        if len(applied_refinement.decompos)>1:
            ...
            for decomp in applied_refinement.decompos:
                ...
                if decomp.type == DecompType.BOTH_AGENDAS_EMPTY:
                    ...
                    # Create incomplete node (previous, cost, depth, next, flag E)
                    next_node = create_incomplete_node_dec(decomp, node, current_cost, node.depth)
                    node.next_nodes.append(next_node)

                    if checkInactivityDeadlockDec(decomp):
                        node.Flag = Node.Flag.D
                    else:
                        next_node.flag = Node.Flag.U
...
def checkInactivityDeadlockDec(decomp):
    ...
    H_planned_actions = decomp.agents[g_human_name].planned_actions
    R_planned_actions = decomp.agents[g_robot_name].planned_actions
...
def get_last_nodes_action(node):
    last_actions=[]

    if node.next_nodes==[]:
        return [node.end_action]
...
def get_last_nodes(node):
    ...
    for next in node.next_nodes:
        last_nodes += get_last_nodes(next)
...
def show_belief_alignmen(first_node):
    ...
    last_nodes = get_last_nodes(first_node)

    for node in last_nodes:
        # print("check node")
        if not compare_states(node.end_agents[g_robot_name].state, node.end_agents[g_human_name].state):
            # print("diff")
            diff = True
            # break
        # else:
...
def get_applied_refinement(agent_name, agents, previous_action, current_cost, delaying=False):
    ...
    HUMAN = agent_name==g_human_name
    ROBOT = agent_name==g_robot_name


    if ROBOT and delaying and agents.copresent() and g_with_contrib:
        delaying = False

    if ROBOT and delaying and not agents.copresent() and g_with_contrib:
        if agents[g_human_name].agenda==[]:
            ref = Refinement()
    ...
    if HUMAN and g_with_contrib:
        print("{}- Refine agenda with r_beliefs".format(agent_name))
        refinement_r_beliefs = refine_agenda(agent_name, agents, previous_action.branching_name, agent_state=g_robot_name)
        ...
        if need_belief_alignment(refinement_r_beliefs, refinement):
            ...
            result = find_and_correct_relevant_divergence(agents, g_human_name, refinement_r_beliefs)
            if result == False:
            ...
            refinement = refine_agenda(agent_name, aligned_agents, previous_action.branching_name, agent_state=g_human_name)
    ...
    for decomp in refinement.decompos:
        ...
        if DecompType.NO_APPLICABLE_METHOD == decomp.type:
            ...
            wait_action = Operator.create_inactivity_op("WAIT", g_wait_cost[agent_name], agent_name, previous_action, current_cost)
            ...
            idle_action = Operator.create_inactivity_op("IDLE", g_idle_cost[agent_name], agent_name, previous_action, current_cost)
            ...
            if g_with_contrib and ROBOT and check_diff_effects(newagents, action):
                # find relevant divergence to correct (for now this the current domain structure assumption that only one divergence fixes)
                result = find_div_for_same_eff(agents, g_human_name, action)
                if result == False:
            ...
            if OpType.OPERATOR_NOT_APPLICABLE == result:
                ...
                if not Operator.precond(newagents, newagents[g_other_agent_name[agent_name]].state, action):
                ...
                if g_with_contrib:
                    if ROBOT:
                        ...
                        Operator.apply(newagents, action, agent_name, agent_state_in=g_other_agent_name[agent_name], agent_state_out=g_other_agent_name[agent_name])
                else:
                    if ROBOT:
                        Operator.apply(newagents, action, agent_name, agent_state_in=g_other_agent_name[agent_name], agent_state_out=g_other_agent_name[agent_name])
                    elif HUMAN:
                        res = Operator.apply(newagents, action, agent_name, agent_state_in=g_other_agent_name[agent_name], agent_state_out=g_other_agent_name[agent_name])
                        if OpType.OPERATOR_NOT_APPLICABLE == res:
                            print(bcolors.WARNING + str(action) + " not applicable with ground truth" + bcolors.ENDC)
    ...
    for dec in refinement.decompos:
        if dec.type==DecompType.OK:
            dec.next_action.agent_agendas[g_robot_name] = dec.agents[g_robot_name].agenda
            dec.next_action.agent_agendas[g_human_name] = dec.agents[g_human_name].agenda
    return refinement
...
def refine_agenda(agent_name, in_agents, branching_name, agent_state=None):
    ...
    other_agent_name = g_other_agent_name[agent_name]
            ...
            while not current_decomp.first_task_is_operator_not_done(agent_name, agent_state):
                ...
                # There are applicable methods
                    ...
                    if list_decompo[0].subtasks==[]:
                        print(bcolors.OKGREEN + "\trefines into nothing" + bcolors.ENDC)
                        need_continue = True
                    # If next task is an operator already done
                        ...
                        print(bcolors.OKGREEN + "\talready done" + bcolors.ENDC)
                        need_continue = True
                    if need_continue:
                        # print("NEED_CONTINUE")
...
def compute_fcost(node):
    fcost = None
    i = None
    if node.type==Node.Type.F or node.type==Node.Type.I or node.type==Node.Type.D:
        fcost = node.cost
    elif node.type==Node.Type.R:
        next_costs = []
        for next in node.next_nodes:
            next_costs.append(compute_fcost(next)[0])
        fcost = min(next_costs)
        i = min(range(len(next_costs)), key=next_costs.__getitem__)
        node.selected_next = node.next_nodes[i].id
    elif node.type==Node.Type.H:
        total = 0
        for next in node.next_nodes:
            total += compute_fcost(next)[0]
        fcost = total/len(node.next_nodes)
    node.fcost = fcost
    return (fcost, i)
...
def situation_assessment(agents):
    ...
    ground_truth = agents[g_robot_name].state
    human_state = agents[g_human_name].state
...
# def apply_inferable_effects(agents, state_before):
#     state_after = agents[g_robot_name].state
#     modifs = StateModifications(state_before, state_after)
#     human_state = agents[g_human_name].state
#     for modif in modifs.modifications:
#         if agents.get_fluent_obs(modif.property)==ObsType.INF and agents.get_fluent_loc("at_robot")==agents.get_fluent_loc("at_human"):
#             print("human attended inferable effects! {} <- {}".format(modif.property, modif.next_value))
#             if modif.key == None:
...
def apply_inferable_effects(agents, action):
    ...
    # Operator.apply(cp_agents, action, g_robot_name, agent_state_in=g_human_name, agent_state_out=g_human_name)
    Operator.apply(agents, action, g_robot_name, agent_state_in=g_human_name, agent_state_out=g_human_name)

    # modif = StateModifications(agents[g_human_name].state, cp_agents[g_human_name].state)
    modif = StateModifications(agents[g_human_name].state, agents[g_human_name].state)
...
def check_diff_effects(agents, action):
    ...
    result_h = Operator.apply(agents_h, action, action.agent, agent_state_in=g_human_name, agent_state_out=g_robot_name)
    if result_h!=OpType.OPERATOR_NOT_APPLICABLE:
        result_h = Operator.apply(agents_h, action, action.agent, agent_state_in=g_human_name, agent_state_out=g_human_name)
    result_r = Operator.apply(agents_r, action, action.agent, agent_state_in=g_robot_name, agent_state_out=g_human_name)
    if result_r!=OpType.OPERATOR_NOT_APPLICABLE:
        result_r = Operator.apply(agents_r, action, action.agent, agent_state_in=g_robot_name, agent_state_out=g_robot_name)
    ...
    if result_h==OpType.OPERATOR_NOT_APPLICABLE or result_r==OpType.OPERATOR_NOT_APPLICABLE:
        ...
        # Check states
        state_human_previous = agents[g_human_name].state
        state_robot_previous = agents[g_robot_name].state
        state_human_h_beliefs = agents_h[g_human_name].state
        state_robot_h_beliefs = agents_h[g_robot_name].state
        state_human_r_beliefs = agents_r[g_human_name].state
        state_robot_r_beliefs = agents_r[g_robot_name].state
