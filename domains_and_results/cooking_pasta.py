...
def dec_cook_R(agents, state, agent):
    return [("come_add_salt",), ("come_turn_on_pot_fire",)]

def dec_cook_H(agents, state, agent):
...
def initDomain(n):
    ...
    htpa.add_tasks(robot_name, [("cook",)])
    ...
    # Starting Agent
    htpa.set_starting_agent(values["starting_agent"])
...
def node_explo(with_contrib, with_graph, with_delay, n):
    ...
    view = False
    if with_graph:
        htpa.gui.show_all(htpa.get_last_nodes_action(first_node), robot_name, human_name, n, with_contrib, with_delay, with_begin="false", with_abstract="false", view=view)
